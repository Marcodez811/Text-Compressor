...
Tree* buildTree(uint64_t* data) {
    ...
    for (size_t i = 0; i < ASCII_RANGE; i++) {
        if (data[i] > 0) {
            Node* n = createNode(data[i], i, NULL, NULL);
            insertPQ(pq, createTree(n));
        }
    }
    while (pq->size > 1) {
...
FILE* compress(FILE* fp, const char* name) {
    ...
    while ((c = fgetc(fp)) != EOF) {
        ...
        for (size_t i = 0; i < len; i++) {
            if (bin_str[i] == '1') {
                buffer |= (1 << (7 - buffer_idx));
            }
            buffer_idx++;
            if (buffer_idx == 8) {
                // write the byte into file
                fwrite(&buffer, 1, 1, new_file);
                // reset buffer information
                buffer = 0, buffer_idx = 0;
            }
        }
    }
    // write the remaining byte into buffer 
    if (buffer_idx > 0) {
        fwrite(&buffer, 1, 1, new_file);
        buffer = 0, buffer_idx = 0;
    }
